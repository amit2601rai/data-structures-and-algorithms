...
class myStack{
    ...
    public:
        void push(int x){
            ...
            else minElement.push(min(minElement.top(), x));
            if(maxElement.empty()) maxElement.push(x);
            else maxElement.push(max(maxElement.top(), x));
        }
        ...
        int top(){
            if(!elements.empty()) return elements.top();
            else printf("Stack Is Empty !!!");
        }

        int getMin(){
            if(!minElement.empty()) return minElement.top();
            else printf("Stack Is Empty !!!");
        }

        int getMax(){
            if(!maxElement.empty()) return maxElement.top();
            else printf("Stack Is Empty !!!");
        }
};

...
void lazyUpdate(int root, int a, int b, int low, int high, int v, Node tree[]) {
  ...
  if (a <= mid) lazyUpdate(2 * root, a, b, low, mid, v, tree);
  if (b > mid) lazyUpdate(2 * root + 1, a, b, mid + 1, high, v, tree);
...
ll query(int root, int a, int b, int low, int high, Node tree[]) {
  ...
  ll lsum = 0, rsum = 0;
  if (a <= mid) lsum = query(2 * root, a, b, low, mid, tree);
  if (b > mid) rsum = query(2 * root + 1, a, b, mid + 1, high, tree);
  return lsum + rsum;
}

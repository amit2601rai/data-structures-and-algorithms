...
void updatetree(int root, int low, int high, int u, int v) {
  ...
  if (u <= mid) updatetree(2 * root, low, mid, u, v);
  if (v > mid) updatetree(2 * root + 1, mid + 1, high, u, v);
...
node querytree(int root, int low, int high, int u, int v) {
  ...
  int mid = (low + high) / 2;
  if (v < low || u > high) return left;
  if (u <= low && high <= v) return tree[root];
  if (tree[root].toggle != 0) {
    tree[2 * root].toggle += tree[root].toggle;
    tree[2 * root + 1].toggle += tree[root].toggle;
    if (tree[root].toggle % 2 != 0) {
  ...
  if (u <= mid) left = querytree(2 * root, low, mid, u, v);
  if (v > mid) right = querytree(2 * root + 1, mid + 1, high, u, v);

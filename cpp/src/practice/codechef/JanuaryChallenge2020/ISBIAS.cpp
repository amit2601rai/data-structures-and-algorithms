...
int main() {
  ...
  while (q--) {
    ...
    if (l == 0)
      maxInc = inc[r];
    else if (elem[l - 1] < elem[l] && (l < n - 1 && elem[l] < elem[l + 1]))
      maxInc = inc[r] - inc[l - 1] + (inc[l] == inc[l - 1] ? 1 : 0);
    else if (elem[l - 1] < elem[l] && (l < n - 1 && elem[l] > elem[l + 1]))
      maxInc = inc[r] - inc[l];
    else
      maxInc = inc[r] - inc[l - 1];

    if (l == 0)
      maxDec = dec[r];
    else if (elem[l - 1] > elem[l] && (l < n - 1 && elem[l] > elem[l + 1]))
      maxDec = dec[r] - dec[l - 1] + (dec[l] == dec[l - 1] ? 1 : 0);
    else if (elem[l - 1] > elem[l] && (l < n - 1 && elem[l] < elem[l + 1]))
      maxDec = dec[r] - dec[l];
    else
      maxDec = dec[r] - dec[l - 1];

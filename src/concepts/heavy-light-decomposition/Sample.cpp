...
void make_tree(int cur, int s, int e) {
  ...
  int c1 = (cur << 1), c2 = c1 | 1, m = (s + e) >> 1;  // Awesome piece of code x | 1 = x if x is odd, x + 1 if x is
                                                       // even
  make_tree(c1, s, m);
  make_tree(c2, m, e);
...
void HLD(int curNode, int cost, int prev) {
  ...
  chainInd[curNode] = chainNo;  // Head Node : Chain Number
  ...
  // If special child has not been already found then recurse with special child
  if (sc != -1) HLD(sc, ncost, curNode);

  // Starting the HLD process from allover again
  for (int i = 0; i < adj[curNode].size(); i++)
    if (adj[curNode][i] != prev) {
      if (sc != adj[curNode][i]) {
...
void dfs(int cur, int prev, int _depth = 0) {
  ...
  for (int i = 0; i < adj[cur].size(); i++)
    if (adj[cur][i] != prev) {
      otherEnd[indexx[cur][i]] = adj[cur][i];  //? use of this

...
bool comparator(struct box a, struct box b) { return (a.area > b.area); }
...
int main() {
  while (1) {
    ...
    int temp = -1;
    int ans[3 * n + 1];
    for (int i = 1; i <= 3 * n; i++) ans[i] = boxes[i].height;
    // for(int i=1;i<=3*n;i++)
    //   cout<<boxes[i].height<<" ";
    // cout<<endl;
    ...
    for (int i = 1; i <= 3 * n; i++) {
      ...
      for (int j = i - 1; j >= 1; j--) {
        ...
        if (boxes[j].length > boxes[i].length && boxes[j].breadth > boxes[i].breadth) {
          //		cout<<"j ="<<j<<" "<<"i= "<<i<<endl;
          //		cout<<"("<<boxes[j].length<<" "<<boxes[j].breadth<<")
          //("<<boxes[i].length<<boxes[i].breadth<<")"<<endl;

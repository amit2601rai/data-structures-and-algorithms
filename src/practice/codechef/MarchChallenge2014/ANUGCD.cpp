...
int b_search(int key, int root) {
  ...
  while (low <= high) {
    ...
    if (tree[root].v[mid].primefactor < key)
      low = mid + 1;
    else
      high = mid - 1;
  }
...
ii Query(int root, int low, int high, int g, int u, int v) {
  ...
  int mid = (low + high) / 2;
  if (v < low || u > high) return e_left;
  if (u <= low && high <= v) return eval(g, root);
  if (u <= mid) e_left = Query(2 * root, low, mid, g, u, v);
  if (v > mid) e_right = Query(2 * root + 1, mid + 1, high, g, u, v);
